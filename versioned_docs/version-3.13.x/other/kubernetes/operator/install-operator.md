...
```bash
docker pull rabbitmqoperator/cluster-operator:&lcub;some-version}
docker tag rabbitmqoperator/cluster-operator:&lcub;some-version} &lcub;someregistry}/cluster-operator:&lcub;some-version}
docker push &lcub;someregistry}/cluster-operator:&lcub;some-version}
```

The value of `&lcub;someregistry}` should be the address of an OCI compatible registry. The value of `&lcub;some-version}` is
...
rabbitmq-cluster-operator -p '&lcub;"imagePullSecrets": [&lcub;"name": "rabbitmq-cluster-registry-access"}]}'
```
