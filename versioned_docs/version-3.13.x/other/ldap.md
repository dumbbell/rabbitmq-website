...
&lcub;rabbit, [
  &lcub;auth_backends, [rabbit_auth_backend_ldap]}
]}
</pre>
...
<pre class="lang-erlang">
&lcub;rabbit,[
  &lcub;auth_backends, [rabbit_auth_backend_ldap, rabbit_auth_backend_internal]}
]}
</pre>
...
<pre class="lang-erlang">
&lcub;rabbit,[&lcub;auth_backends, [&lcub;rabbit_auth_backend_ldap, rabbit_auth_backend_internal},
...
<pre class="lang-erlang">
[
  &lcub;rabbitmq_auth_backend_ldap, [
    &lcub;servers, ["ldap.eng.megacorp.local", "192.168.0.100"]}
  ]}
].
</pre>
...
[
  &lcub;rabbitmq_auth_backend_ldap, [
     &lcub;ssl_options, [&lcub;cacertfile,"/path/to/ca_certificate.pem"},
...
passes the provided value as-is (i.e. the pattern is `$&lcub;username}`). If
...
[
  &lcub;rabbitmq_auth_backend_ldap, [
    &lcub;other_bind, &lcub;"a-username", "a-password"}}
  ]}
].
</pre>
...
the `&lcub;in_group_nested, ...}` query only. For more info see the [section on queries](#query-reference).
...
[
  &lcub;rabbitmq_auth_backend_ldap, [
    %% ...
    &lcub;group_lookup_base, "ou=groups,dc=example,dc=com"}
  ]}
]
</pre>
...
<table>
  ...
  <tr>
    ...
    <td><code>&lcub;constant, true}</code></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><code>[&lcub;administrator, &lcub;constant, false}}]</code></td>
  </tr>
</table>
 ...
 * `$&lcub;username}`: the user name provided at authentication
 * `$&lcub;user_dn}`: the distinguished name of the user
 * `$&lcub;vhost}`: the virtual host for which we are querying access

The `resource_access_query` supports:

 * `$&lcub;username}`: the user name provided at authentication
 * `$&lcub;user_dn}`: the distinguished name of the user
 * `$&lcub;vhost}`: the virtual host in which the resource resides
 * `$&lcub;resource}`: one of "exchange" or "queue" for the type of resource
 * `$&lcub;name}`: the name of the resource
 * `$&lcub;permission}`: one of "configure", "write" or "read" for the type of access being requested to the resource

The `tag_queries` supports:

 * `$&lcub;username}`: the user name provided at authentication
 * `$&lcub;user_dn}`: the distinguished name of the user
 * `$&lcub;vhost}`: **virtual host information will not be available in every scenario**. It can be used for additional context,
 ...
 * `$&lcub;user_dn}`: the distinguished name of the user
 * `$&lcub;vhost}`: the virtual host in which the resource resides
 * `$&lcub;resource}`: always "topic" in this case
 * `$&lcub;name}`: the name of the resource
 * `$&lcub;permission}`: one of "write" (publishing) or "read" (consuming, queue and
   exchange-to-exchange binding for topic exchanges)
 * `$&lcub;routing_key}`: the routing key of the published message ("write" permission)
 ...
 * `$&lcub;ad_domain}` - the domain part of `Domain\User`
 * `$&lcub;ad_user}` - the user part of `Domain\User`
...
<pre class="lang-erlang">&lcub;constant, Bool}</pre>

This will always return either true or false, unconditionally granting
or denying access. Example:

<pre class="lang-erlang">
&lcub;tag_queries, [&lcub;administrator, &lcub;constant, false}},
 &lcub;management,    &lcub;constant, true}}]}
</pre>
...
<pre class="lang-erlang">&lcub;exists, Pattern}</pre>

This will substitute variables into the pattern, and return true if
there exists an object with the resulting DN. Example:

<pre class="lang-erlang">&lcub;vhost_access_query, &lcub;exists, "ou=$&lcub;vhost},ou=vhosts,dc=example,dc=com"}}</pre>
...
<pre class="lang-erlang">&lcub;in_group, Pattern}</pre>
<pre class="lang-erlang">&lcub;in_group, Pattern, AttributeName}</pre>
...
<pre class="lang-erlang">&lcub;vhost_access_query, &lcub;in_group, "cn=$&lcub;vhost}-users,ou=vhosts,dc=example,dc=com"}}</pre>
...
<pre class="lang-erlang">&lcub;in_group_nested, Pattern}</pre>
<pre class="lang-erlang">&lcub;in_group_nested, Pattern, AttributeName}</pre>
<pre class="lang-erlang">&lcub;in_group_nested, Pattern, AttributeName, Scope}</pre>
...
recommended to use plain `&lcub;in_group, ...}` query
...
[
  &lcub;group_lookup_base, "ou=groups,dc=example,dc=com"},
  &lcub;vhost_access_query, &lcub;in_group_nested, "cn=$&lcub;vhost}-groups,ou=groups,dc=example,dc=com"}, "member", single_level}
]
</pre>
...
<pre class="lang-erlang">&lcub;for, [&lcub;Name, Value, SubQuery}, ...]}</pre>
...
(i.e. something that would go into a `$&lcub;}`
...
<pre class="lang-erlang">
&lcub;resource_access_query,
 &lcub;for, [&lcub;resource, exchange, &lcub;for, [&lcub;permission, configure,
                                     &lcub;in_group, "cn=wheel,dc=example,dc=com"}
                                    },
                                    &lcub;permission, write, &lcub;constant, true}},
                                    &lcub;permission, read,  &lcub;constant, true}}
                                   ]}},
                                   &lcub;resource, queue,    &lcub;constant, true}}]}}
</pre>
...
<pre class="lang-erlang">&lcub;'not', SubQuery}</pre>
<pre class="lang-erlang">&lcub;'and', [SubQuery1, SubQuery2, SubQuery3, ...]}</pre>
<pre class="lang-erlang">&lcub;'or', [SubQuery1, SubQuery2, SubQuery3, ...]}</pre>
...
<pre class="lang-erlang">&lcub;resource_access_query,
 &lcub;'or',
  [&lcub;'and',
    [&lcub;equals, "$&lcub;name}", "test1"},
     &lcub;equals, "$&lcub;username}", "user1"}]},
   &lcub;'and',
    [&lcub;equals, "$&lcub;name}", "test2"},
     &lcub;'not', &lcub;equals, "$&lcub;username}", "user1"}}]}
  ]}}
</pre>
...
<pre class="lang-erlang">&lcub;equals, StringSubQuery1, StringSubQuery2}</pre>
...
<pre class="lang-erlang">&lcub;resource_access_query,
 &lcub;for, [&lcub;permission, configure, &lcub;equals, &lcub;attribute, "$&lcub;user_dn}", "description"},
                                         &lcub;string, "can-declare-$&lcub;resource}s"}
                                }
        },
        &lcub;permission, write, &lcub;constant, true}},
        &lcub;permission, read,  &lcub;constant, true}}
...
<pre class="lang-erlang">&lcub;match, StringSubQuery, RESubQuery}</pre>
...
<pre class="lang-erlang">
&lcub;resource_access_query, &lcub;match, &lcub;string, "$&lcub;name}"},
                      &lcub;string, "^$&lcub;username}-"}}
}</pre>
...
<pre class="lang-erlang">&lcub;string, Pattern}</pre>
...
<pre class="lang-erlang">&lcub;attribute, DNPattern, AttributeName}</pre>
...
auth_ldap.user_dn_pattern = cn=$&lcub;username},ou=People,dc=example,dc=com
...
<pre class="lang-erlang">
[&lcub;rabbitmq_auth_backend_ldap,[
    &lcub;vhost_access_query,    &lcub;in_group,
                              "ou=$&lcub;vhost}-users,ou=vhosts,dc=example,dc=com"}},
     &lcub;resource_access_query,
      &lcub;for, [&lcub;permission, configure, &lcub;in_group, "cn=admin,dc=example,dc=com"}},
             &lcub;permission, write,
              &lcub;for, [&lcub;resource, queue,    &lcub;in_group, "cn=admin,dc=example,dc=com"}},
                     &lcub;resource, exchange, &lcub;constant, true}}]}},
             &lcub;permission, read,
              &lcub;for, [&lcub;resource, exchange, &lcub;in_group, "cn=admin,dc=example,dc=com"}},
                     &lcub;resource, queue,    &lcub;constant, true}}]}}
            ]
      }},
     &lcub;topic_access_query,
      &lcub;for, [&lcub;permission, write, &lcub;match, &lcub;string, "$&lcub;routing_key}"}, &lcub;string, "^a"}}},
             &lcub;permission, read,  &lcub;constant, true}}
            ]
      }},
     &lcub;tag_queries,           [&lcub;administrator, &lcub;constant, false}},
...
<pre class="lang-erlang">
[
  &lcub;rabbit, [&lcub;auth_backends, [rabbit_auth_backend_ldap]}]},
  &lcub;rabbitmq_auth_backend_ldap,
   [ &lcub;servers,               ["my-ldap-server"]},
     &lcub;user_dn_pattern,       "cn=$&lcub;username},ou=People,dc=example,dc=com"},
     &lcub;use_ssl,               false},
     &lcub;port,                  389},
     &lcub;log,                   false},
     &lcub;vhost_access_query,    &lcub;in_group,
