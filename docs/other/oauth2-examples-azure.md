...
description: Use Azure Active Directory (Azure AD) as OAuth 2.0 server
weight: 10
---
...
7. On the right pane that has just opened, copy the value of **OpenID Connect metadata document** (ex: `https://login.microsoftonline.com/&lcub;TENANT_ID}/v2.0/.well-known/openid-configuration`) and open it in your browser.

    Note the value of the `jwks_uri` key (ex: `https://login.microsoftonline.com/&lcub;TENANT_ID}/discovery/v2.0/keys`), as you will also need it later to configure the `rabbitmq_auth_backend_oauth2` on RabbitMQ side.
...
For example, given your application id, `&lcub;my-app-id}` and your tenant `&lcub;tenant}`, the OIDC discovery endpoint uri would be `https://login.microsoftonline.com/&lcub;tenant}/.well-known/openid-configuration?appid=&lcub;my-app-id}`. The returned payload contains the `jwks_uri` attribute whose value is something like `https://login.microsoftonline.com/&lcub;tenant}/discovery/keys?appid=<my-app-idp>`. RabbitMQ should be configured with that `jwks_uri` value.
...
* Value of the **jwks_uri** key from `https://login.microsoftonline.com/&lcub;TENANT_ID}/v2.0/.well-known/openid-configuration`

<pre class="lang-bash">
$ vi rabbitmq.config
[
  &lcub;rabbit, [
   &lcub;auth_backends, [rabbit_auth_backend_oauth2, rabbit_auth_backend_internal]}
  ]},
  &lcub;rabbitmq_management, [
     &lcub;oauth_enabled, true},
     &lcub;oauth_client_id, "PUT YOUR AZURE AD APPLICATION ID"},
     &lcub;oauth_provider_url, "https://login.microsoftonline.com/AZURE_AD_TENANT_ID"}
 ]},
 &lcub;rabbitmq_auth_backend_oauth2, [
   &lcub;resource_server_id, &lt;&lt;"PUT YOUR AZURE AD APPLICATION ID"&gt;&gt;},
   &lcub;extra_scopes_source, &lt;&lt;"roles"&gt;&gt;},
   &lcub;key_config, [
     &lcub;jwks_url, &lt;&lt;"PUT YOUR AZURE AD JWKS URI VALUE"&gt;&gt;}
   ]}
 ]}
].
</pre>
...
You have configured RabbitMQ with `&lcub;extra_scopes_source, <<"roles">>},` which means RabbitMQ uses
