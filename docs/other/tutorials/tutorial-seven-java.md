...
<pre class="lang-java">
while (thereAreMessagesToPublish()) &lcub;
    byte[] body = ...;
...
while (thereAreMessagesToPublish()) &lcub;
    ...
    if (outstandingMessageCount == batchSize) &lcub;
        channel.waitForConfirmsOrDie(5_000);
        outstandingMessageCount = 0;
    }
}
if (outstandingMessageCount > 0) &lcub;
    channel.waitForConfirmsOrDie(5_000);
}
</pre>
...
Channel channel = connection.createChannel();
channel.confirmSelect();
channel.addConfirmListener((sequenceNumber, multiple) -> &lcub;
    // code when message is confirmed
}, (sequenceNumber, multiple) -> &lcub;
...
<pre class="lang-java">
ConcurrentNavigableMap&lt;Long, String&gt; outstandingConfirms = new ConcurrentSkipListMap&lt;&gt;();
ConfirmCallback cleanOutstandingConfirms = (sequenceNumber, multiple) -> &lcub;
    if (multiple) &lcub;
...
channel.addConfirmListener(cleanOutstandingConfirms, (sequenceNumber, multiple) -> &lcub;
...
<pre class="lang-bash">
static Connection createConnection() throws Exception &lcub;
