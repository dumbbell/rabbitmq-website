...
func randomString(l int) string &lcub;
        bytes := make([]byte, l)
        for i := 0; i &lt; l; i++ &lcub;
...
func randInt(min int, max int) int &lcub;
        return min + rand.Intn(max-min)
}

func fibonacciRPC(n int) (res int, err error) &lcub;
        conn, err := amqp.Dial("amqp://guest:guest@localhost:5672/")
        failOnError(err, "Failed to connect to RabbitMQ")
        defer conn.Close()
        ...
        for d := range msgs &lcub;
                if corrId == d.CorrelationId &lcub;
...
func bodyFrom(args []string) int &lcub;
        var s string
