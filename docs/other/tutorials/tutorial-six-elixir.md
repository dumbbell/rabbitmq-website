...
<pre class="lang-elixir">
&lcub;:ok, %&lcub;queue: callback_queue}} = AMQP.Queue.declare(channel,
                                                     "",
                                                     exclusive: true)
  ...
  def wait_for_messages(channel) do
    receive do
      &lcub;:basic_deliver, payload, meta} ->
        &lcub;n, _} = Integer.parse(payload)
        IO.puts " [.] fib(#&lcub;n})"
        response = fib(n)
...
defmodule FibonacciRpcClient do
  def wait_for_messages(_channel, correlation_id) do
    receive do
      &lcub;:basic_deliver, payload, %&lcub;correlation_id: ^correlation_id}} ->
        &lcub;n, _} = Integer.parse(payload)
        n
    end
  end
  def call(n) do
    ...
    &lcub;:ok, %&lcub;queue: queue_name}} = AMQP.Queue.declare(channel,
                                                     "",
                                                     exclusive: true)
...
IO.puts " [x] Requesting fib(#&lcub;num})"
response = FibonacciRpcClient.call(num)
IO.puts " [.] Got #&lcub;response}"
</pre>
