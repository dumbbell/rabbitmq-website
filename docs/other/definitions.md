...
curl -u &lcub;username}:&lcub;password} -X GET http://&lcub;hostname}:15672/api/definitions
</pre>
...
curl -u &lcub;username}:&lcub;password} -X GET http://&lcub;hostname}:15672/api/definitions | jq
</pre>
...
# placeholders are used for credentials and hostname.
# Use HTTPS when possible.
curl -u &lcub;username}:&lcub;password} -H "Content-Type: application/json" -X POST -T /path/to/definitions.file.json http://&lcub;hostname}:15672/api/definitions
</pre>
