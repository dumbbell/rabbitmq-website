...
spec:
  rabbitmq:
    advancedConfig: |
      [
          &lcub;ra, [
              &lcub;wal_data_dir, '/var/lib/rabbitmq/quorum-wal'}
          ]}
      ].
```
...
-ojsonpath='Name: &lcub;.status.admin.serviceReference.name} -- Namespace: &lcub;.status.admin.serviceReference.namespace}'
```
...
run `kubectl get rabbitmqcluster INSTANCE -ojsonpath='&lcub;.status.defaultUser.secretReference.name}'`

To retrieve credentials and display them in plaintext, first display the username by running:

```bash
kubectl -n NAMESPACE get secret INSTANCE-default-user -o jsonpath="&lcub;.data.username}" | base64 --decode
```
...
```bash
kubectl -n NAMESPACE get secret INSTANCE-default-user -o jsonpath="&lcub;.data.password}" | base64 --decode
```
...
username=$(kubectl get secret $&lcub;instance}-default-user -o jsonpath="&lcub;.data.username}" | base64 --decode)
password=$(kubectl get secret $&lcub;instance}-default-user -o jsonpath="&lcub;.data.password}" | base64 --decode)
service=$&lcub;instance}
kubectl run perf-test --image=pivotalrabbitmq/perf-test -- --uri "amqp://$&lcub;username}:$&lcub;password}@$&lcub;service}"
```
